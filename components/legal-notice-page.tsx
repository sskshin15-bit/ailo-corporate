    ...
    <main className="bg-[linear-gradient(180deg,#0b1c38_0%,#101f3a_55%,#14274a_100%)] pt-28 pb-20">
      <section className="mx-auto max-w-4xl px-6 lg:px-10">
        ...
        <dl className="mt-10 space-y-5">
          {t.items.map((item) => (
            <div key={item.term} className="border-l border-primary/40 pl-5">
              <dt className="text-lg font-medium text-foreground">{item.term}</dt>
              <dd className="mt-2 text-sm font-light leading-relaxed text-muted-foreground">{item.desc}</dd>
            </div>
          ))}
        </dl>

        <article className="mt-10 border-l border-primary/40 pl-5">
          <h2 className="text-lg font-medium text-foreground">{t.notesTitle}</h2>
          <p className="mt-3 text-sm font-light leading-relaxed text-muted-foreground">{t.notes}</p>
        </article>
